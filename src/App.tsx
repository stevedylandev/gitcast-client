...
function App() {
	...
	const [activeRepoFilter, setActiveRepoFilter] = useState<
		"stars" | "farcaster" | null
	>(null);
	...
	const sortedRepositories = useMemo(() => {
		...
		if (activeRepoFilter === "stars") {
			...
			return repos.sort(
				(a, b) => b.farcaster_stars_count - a.farcaster_stars_count,
			);
		}
			...
			<div className="flex flex-col items-center justify-center min-h-screen gap-4">
				...
				<Card className="max-w-sm">
					...
					<CardContent className="text-center">
						<p className="mb-4">
							We're currently indexing your GitHub data. This process takes a
							minute to complete.
						</p>
						<p className="font-medium">
							Please come back in a minute to see your feed!
						</p>
					</CardContent>
				</Card>
			</div>
			...
			<Tabs defaultValue="feed" className="w-full">
				...
				<TabsContent value="feed">
					<div className="flex flex-wrap justify-center gap-0 mb-6">
						...
						>
							<Badge
								variant={activeFilters.PushEvent ? "pushEvent" : "outline"}
								className="flex items-center gap-1 text-xs"
							>
						...
						>
							<Badge
								variant={
									activeFilters.PullRequestEvent
										? "pullRequestEvent"
										: "outline"
								}
								className="flex items-center gap-1 text-xs"
						...
						>
							<Badge
								variant={activeFilters.WatchEvent ? "watchEvent" : "outline"}
								className="flex items-center gap-1 text-xs"
							>
						...
						>
							<Badge
								variant={activeFilters.CreateEvent ? "createEvent" : "outline"}
								className="flex items-center gap-1 text-xs"
							>
										...
										<div className="flex items-start gap-3">
											<Link href={`https://github.com/${event.actor.login}`}>
												<Avatar className="h-10 w-10 rounded-full border-2 border-border">
													<img
														src={
															event.farcaster.pfp_url || event.actor.avatar_url
														}
											...
											<div className="flex-1 min-w-0">
												<div className="flex items-center gap-2">
													<span className="font-semibold">
														{event.farcaster.display_name || event.actor.login}
													</span>
													<Badge
												...
												<div className="text-sm text-muted-foreground mt-1 overflow-hidden text-ellipsis">
													<p className="underline">{event.repo.name}</p>
												</div>
												{event.commitMessage && (
													<p className="mt-2 text-sm bg-muted p-2 rounded-md border border-border overflow-hidden">
														<span className="line-clamp-3 break-words font-mono text-xs">
															{event.commitMessage}
														</span>
													</p>
												)}
				...
				<TabsContent value="repos">
					<div className="flex flex-wrap justify-center gap-0 mb-6">
						...
						>
							<Badge
								variant={
									activeRepoFilter === "stars" ? "watchEvent" : "outline"
								}
								className="flex items-center gap-1 text-xs"
							>
													...
													<div className="flex items-center gap-4 mt-3">
														...
														{repo.farcaster_stars_count > 0 && (
															<Badge variant="secondary" className="text-xs">
																{repo.farcaster_stars_count} Farcaster{" "}
																{repo.farcaster_stars_count === 1
																	? "user"
																	: "users"}
															</Badge>
														)}
